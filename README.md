...
사용자는 단순히 이미지를 올리고, 팔레트를 선택하고, 필요하다면 선 위에 가이드 색을 조금 찍어주는 것만으로 귀여운 컬러 이미지를 얻을 수 있습니다.
...
- 배경으로 판단되는 영역은 흰색으로 유지하여 **배경이 덜렁 채색되는 문제**를 줄였습니다.
...
- 가이드가 없는 영역은 **흰색(미채색 상태)**로 남기기 때문에, 필요하지 않은 부분이 자동으로 엉뚱하게 칠해지지 않습니다.
...
*(실제 GitHub에서는 스크린샷 이미지를 예를 들어 `docs/` 또는 `images/` 폴더에 함께 올려두면 좋습니다.)*
...
README에 스크린샷을 다음과 같이 첨부할 수 있습니다.

```md
![기본 페이지](./docs/screenshot_main.png)
```

> 실제 파일명과 경로는 GitHub에 업로드한 이미지 이름에 맞게 수정해 주세요.
...
```md
![팔레트 기반 자동 채색 (가이드 없음)](./docs/screenshot_no_guide.png)
```
...
6. 결과 이미지는 README에서 다음과 같이 첨부할 수 있습니다.

```md
![가이드 기반 자동 채색](./docs/screenshot_with_guide.png)
```
...
이렇게 해서 배경은 항상 흰색으로 유지되며, 불필요한 배경 채색을 방지합니다.
